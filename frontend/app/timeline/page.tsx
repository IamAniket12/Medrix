...
function StatCard({ icon: Icon, value, label, color }: { icon: any; value: number | string; label: string; color: string }) {
  return (
    <div style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.07)", borderRadius: 16, padding: "18px 22px", display: "flex", alignItems: "center", gap: 14, flex: 1, minWidth: 140 }}>
      ...
      <div>
        <div style={{ fontSize: 24, fontWeight: 700, color: "#1c1917", lineHeight: 1 }}>{value}</div>
        <div style={{ fontSize: 11, color: "#a8a29e", marginTop: 4 }}>{label}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
      <svg width={88} height={88} viewBox="0 0 88 88">
        <circle cx={44} cy={44} r={r} fill="none" stroke="rgba(0,0,0,0.1)" strokeWidth={8} />
        ...
        <text x={44} y={58} textAnchor="middle" fill="#a8a29e" fontSize={12}>{grade}</text>
      </svg>
      <div>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#1c1917" }}>Health Score</div>
        <div style={{ fontSize: 11, color: "#a8a29e", marginTop: 2 }}>Based on your records</div>
      </div>
    </div>
  );
}
...
function EventDetail({ detail }: { detail: RelatedDetail }) {
  return (
    <div style={{ marginTop: 14, background: "#ffffff", border: "1px solid rgba(0,0,0,0.06)", borderRadius: 10, overflow: "hidden" }}>
      {detail.condition && (
        <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
          <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: "#a8a29e", marginBottom: 5 }}>Condition</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1c1917", marginBottom: 8 }}>{detail.condition.name}</div>
          ...
          {detail.condition.notes && <div style={{ fontSize: 12, color: "#78716c", marginTop: 8 }}>{detail.condition.notes}</div>}
        </div>
      )}
      {detail.medication && (
        <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
          <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: "#a8a29e", marginBottom: 5 }}>Medication</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1c1917", marginBottom: 8 }}>{detail.medication.name}</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          ...
          {detail.medication.indication && <div style={{ fontSize: 12, color: "#78716c", marginTop: 8 }}>For: {detail.medication.indication}</div>}
          {detail.medication.prescriber && <div style={{ fontSize: 12, color: "#a8a29e", marginTop: 4 }}>By: {detail.medication.prescriber}</div>}
        </div>
      )}
      {detail.lab_result && (
        <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
          <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: "#a8a29e", marginBottom: 5 }}>Lab Result</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1c1917", marginBottom: 8 }}>{detail.lab_result.test_name}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            {detail.lab_result.reference_range && <span style={{ fontSize: 11, color: "#78716c" }}>ref: {detail.lab_result.reference_range}</span>}
            {detail.lab_result.is_abnormal && <Chip text="Abnormal" color="#ef4444" />}
          </div>
          {detail.lab_result.lab_facility && <div style={{ fontSize: 12, color: "#78716c", marginTop: 8 }}>{detail.lab_result.lab_facility}</div>}
        </div>
      )}
      {detail.procedure && (
        <div style={{ padding: "10px 14px" }}>
          <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: "#a8a29e", marginBottom: 5 }}>Procedure</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1c1917", marginBottom: 8 }}>{detail.procedure.procedure_name}</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {detail.procedure.body_site && <Chip text={detail.procedure.body_site} color="#a855f7" />}
          </div>
          {detail.procedure.outcome && <div style={{ fontSize: 12, color: "#78716c", marginTop: 8 }}>Outcome: {detail.procedure.outcome}</div>}
          {detail.procedure.facility && <div style={{ fontSize: 12, color: "#a8a29e", marginTop: 4 }}>{detail.procedure.facility}</div>}
        </div>
      )}
    </div>
  );
...
export default function TimelinePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #fffbf7 0%, #fef3ec 40%, #fff6f0 70%, #fffbf7 100%)", color: "#1c1917", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <style>{`@keyframes spin { to { transform: rotate(360deg) } } select option { background: #ffffff; }`}</style>

      {/* Header */}
      <div style={{ padding: "26px 32px 18px", borderBottom: "1px solid rgba(249,115,22,0.1)", background: "rgba(255,251,247,0.8)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <h1 style={{ fontSize: 21, fontWeight: 700, color: "#1c1917", margin: 0 }}>Medical Timeline</h1>
            <p style={{ fontSize: 13, color: "#78716c", margin: 0 }}>Chronological history across all documents</p>
          </div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => setShowInsights(v => !v)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", background: showInsights ? "rgba(99,102,241,0.15)" : "rgba(99,102,241,0.07)", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 10, color: "#a5b4fc", fontSize: 13, cursor: "pointer", fontWeight: 500 }}>
          ...
          <button onClick={() => setShowFilters(v => !v)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", background: showFilters ? "rgba(16,185,129,0.12)" : "rgba(0,0,0,0.04)", border: `1px solid ${showFilters ? "rgba(16,185,129,0.4)" : "rgba(0,0,0,0.08)"}`, borderRadius: 10, color: showFilters ? "#059669" : "#78716c", fontSize: 13, cursor: "pointer", fontWeight: 500 }}>
          ...
          <button onClick={fetchData} style={{ width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 10, cursor: "pointer" }}>
            <RefreshCw size={15} color="#a8a29e" />
          </button>
        </div>
      </div>
      ...
      {stats && (
        <div style={{ padding: "18px 32px", display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          {insights && (
            <div style={{ flex: 1, minWidth: 200, background: "#ffffff", border: "1px solid rgba(0,0,0,0.07)", borderRadius: 16, padding: "12px 20px", display: "flex", alignItems: "center" }}>
      ...
      {showInsights && insights && (
        <div style={{ margin: "0 32px 20px", background: "rgba(99,102,241,0.05)", border: "1px solid rgba(99,102,241,0.15)", borderRadius: 16, padding: 20, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 }}>
          {(insights.health_score?.insights?.length ?? 0) > 0 && (
            <div>
              ...
              {insights.health_score.insights.map((ins, i) => (
                <div key={i} style={{ fontSize: 13, color: "#44403c", marginBottom: 6 }}>{"bullet"} {ins}</div>
              ))}
            </div>
          )}
            ...
            <div>
              ...
              {insights.predictions.map((p, i) => (
                <div key={i} style={{ fontSize: 13, color: "#fde68a", marginBottom: 8, background: "rgba(245,158,11,0.08)", borderRadius: 8, padding: "8px 10px", border: "1px solid rgba(245,158,11,0.15)" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#9a6700" }}>{p.recommended_action}</div>
                </div>
              ))}
            </div>
          )}
            ...
            <div>
              ...
              {insights.alerts.slice(0, 4).map((a, i) => (
                <div key={i} style={{ fontSize: 13, color: "#fca5a5", marginBottom: 8, background: "rgba(239,68,68,0.08)", borderRadius: 8, padding: "8px 10px", border: "1px solid rgba(239,68,68,0.15)" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#991b1b" }}>{a.recommendation}</div>
                </div>
              ))}
            </div>
          )}
              ...
              {insights.disease_progression.map((d: any, i: number) => {
                ...
                return (
                  <div key={i} style={{ fontSize: 13, marginBottom: 8, padding: "8px 10px", borderRadius: 8, background: `${color}11`, border: `1px solid ${color}22` }}>
                    ...
                    <div style={{ fontSize: 11, color: "#78716c", marginTop: 3 }}>{d.test_name}: {d.first_value} {"->"} {d.last_value}</div>
                  </div>
                );
              })}
            </div>
      ...
      {/* Filter bar */}
      {showFilters && (
        <div style={{ margin: "0 32px 20px", background: "rgba(16,185,129,0.04)", border: "1.5px solid rgba(16,185,129,0.15)", borderRadius: 16, padding: 20, display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", gap: 14, alignItems: "end" }}>
          <div>
            <label style={{ fontSize: 11, color: "#78716c", display: "block", marginBottom: 6 }}>SEARCH</label>
            <div style={{ position: "relative" }}>
              <Search size={13} color="#a8a29e" style={{ position: "absolute", left: 11, top: "50%", transform: "translateY(-50%)" }} />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Events, providers..." style={{ width: "100%", paddingLeft: 32, paddingRight: 10, paddingTop: 9, paddingBottom: 9, background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 9, color: "#1c1917", fontSize: 13, boxSizing: "border-box", outline: "none" }} />
            </div>
          </div>
          <div>
            <label style={{ fontSize: 11, color: "#78716c", display: "block", marginBottom: 6 }}>TYPE</label>
            <select value={filterType} onChange={e => setFilterType(e.target.value)} style={{ width: "100%", padding: "9px 10px", background: "#ffffff", border: "1.5px solid rgba(0,0,0,0.1)", borderRadius: 9, color: "#1c1917", fontSize: 13, outline: "none" }}>
          ...
          <div>
            <label style={{ fontSize: 11, color: "#78716c", display: "block", marginBottom: 6 }}>IMPORTANCE</label>
            <select value={filterImportance} onChange={e => setFilterImportance(e.target.value)} style={{ width: "100%", padding: "9px 10px", background: "#ffffff", border: "1.5px solid rgba(0,0,0,0.1)", borderRadius: 9, color: "#1c1917", fontSize: 13, outline: "none" }}>
          ...
          <div>
            <label style={{ fontSize: 11, color: "#78716c", display: "block", marginBottom: 6 }}>FROM</label>
            <input type="date" value={filterStart} onChange={e => setFilterStart(e.target.value)} style={{ width: "100%", padding: "9px 10px", background: "#ffffff", border: "1.5px solid rgba(0,0,0,0.1)", borderRadius: 9, color: "#1c1917", fontSize: 13, outline: "none", colorScheme: "light", boxSizing: "border-box" }} />
          </div>
          <div>
            <label style={{ fontSize: 11, color: "#78716c", display: "block", marginBottom: 6 }}>TO</label>
            <input type="date" value={filterEnd} onChange={e => setFilterEnd(e.target.value)} style={{ width: "100%", padding: "9px 10px", background: "#ffffff", border: "1.5px solid rgba(0,0,0,0.1)", borderRadius: 9, color: "#1c1917", fontSize: 13, outline: "none", colorScheme: "light", boxSizing: "border-box" }} />
          </div>
          {hasFilters && (
            <button onClick={() => { setSearch(""); setFilterType(""); setFilterImportance(""); setFilterStart(""); setFilterEnd(""); }} style={{ gridColumn: "1/-1", background: "none", border: "none", color: "#78716c", fontSize: 12, cursor: "pointer", textAlign: "left", padding: 0 }}>
              Clear all filters
            </button>
          )}
        </div>
      ...
      <div style={{ display: "flex", padding: "0 32px 60px", gap: 28 }}>
        {years.length > 0 && (
          <div style={{ position: "sticky", top: 20, alignSelf: "flex-start", paddingTop: 4, minWidth: 52 }}>
            <div style={{ fontSize: 10, color: "#a8a29e", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>Year</div>
            {years.map(yr => (
              <button key={yr} onClick={() => yearRefs.current[yr]?.scrollIntoView({ behavior: "smooth", block: "start" })}
                style={{ display: "block", width: "100%", background: "none", border: "none", color: "#78716c", fontSize: 13, fontWeight: 600, padding: "6px 0", cursor: "pointer", textAlign: "left" }}
        ...
        <div style={{ flex: 1 }}>
          {loading && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 0", gap: 16 }}>
              <div style={{ width: 42, height: 42, borderRadius: "50%", border: "3px solid rgba(249,115,22,0.15)", borderTopColor: "#f97316", animation: "spin 0.8s linear infinite" }} />
              <span style={{ fontSize: 14, color: "#78716c" }}>Loading timeline...</span>
            </div>
          )}
          {!loading && error && (
            <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 16, padding: 28, textAlign: "center" }}>
              ...
              <div style={{ color: "dc2626", fontWeight: 600 }}>Failed to load timeline</div>
              <div style={{ color: "#78716c", fontSize: 13, marginTop: 6 }}>{error}</div>
          ...
          {!loading && !error && filtered.length === 0 && (
            <div style={{ background: "#ffffff", border: "1.5px solid rgba(0,0,0,0.06)", borderRadius: 16, padding: 60, textAlign: "center" }}>
              <Calendar size={40} color="#a8a29e" style={{ margin: "0 auto 16px" }} />
              <div style={{ color: "#78716c", fontWeight: 600 }}>No timeline events found</div>
              <div style={{ color: "#a8a29e", fontSize: 13, marginTop: 6 }}>{hasFilters ? "Try removing some filters" : "Upload medical documents to build your timeline"}</div>
            </div>
          )}
          {!loading && !error && years.map(year => (
            <div key={year} ref={el => { yearRefs.current[year] = el; }} style={{ marginBottom: 36 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22, position: "sticky", top: 0, zIndex: 10, paddingTop: 8, paddingBottom: 8, background: "linear-gradient(135deg, #fffbf7 0%, #fef3ec 40%, #fff6f0 70%, #fffbf7 100%)" }}>
                ...
                <div style={{ fontSize: 12, color: "#a8a29e" }}>{Object.values(byYear[year]).flat().length} events</div>
              </div>
              {Object.entries(byYear[year]).map(([month, monthEvents]) => (
                <div key={month} style={{ marginBottom: 24 }}>
                  <div style={{ fontSize: 12, color: "#78716c", marginBottom: 12, fontWeight: 600 }}>{getMonthShort(monthEvents[0].event_date)}</div>
                  <div style={{ position: "relative" }}>
                    <div style={{ position: "absolute", left: 19, top: 0, bottom: 0, width: 2, background: "rgba(0,0,0,0.08)" }} />
                      ...
                      return (
                        <div key={event.id} style={{ position: "relative", paddingLeft: 50, marginBottom: 14 }}>
                          ...
                          <div onClick={() => hasDetail && setExpandedId(isExpanded ? null : event.id)}
                            style={{ background: isExpanded ? `linear-gradient(135deg, ${cfg.bg}, rgba(255,255,255,0.8))` : "#ffffff", border: `1px solid ${isExpanded ? cfg.color + "44" : "rgba(0,0,0,0.07)"}`, borderRadius: 14, padding: "13px 16px", cursor: hasDetail ? "pointer" : "default", transition: "all 0.2s" }}
                            onMouseEnter={e => { if (!isExpanded) (e.currentTarget as HTMLElement).style.borderColor = "rgba(249,115,22,0.15)"; }}
                            onMouseLeave={e => { if (!isExpanded) (e.currentTarget as HTMLElement).style.borderColor = "rgba(0,0,0,0.07)"; }}>
                            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
                              <div style={{ flex: 1 }}>
                                ...
                                <div style={{ fontSize: 14, fontWeight: 600, color: "#1c1917", marginBottom: 3 }}>{event.event_title}</div>
                                <div style={{ fontSize: 12, color: "#78716c" }}>
                                  {fmtDate(event.event_date)}{event.provider && ` - ${event.provider}`}{event.facility && ` - ${event.facility}`}
                                </div>
                                {event.event_description && <div style={{ fontSize: 13, color: "#78716c", marginTop: 7, lineHeight: 1.5 }}>{event.event_description}</div>}
                              </div>
                              {hasDetail && <div style={{ color: "#a8a29e", flexShrink: 0, marginTop: 2 }}>{isExpanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}</div>}
                            </div>
                            {event.document && (
                              <div style={{ marginTop: 8, fontSize: 11, color: "#a8a29e", display: "flex", alignItems: "center", gap: 5 }}>
                                <Info size={10} />{event.document.filename}
                                {event.document.document_type && <span style={{ fontSize: 10, background: "rgba(0,0,0,0.05)", borderRadius: 4, padding: "1px 6px", color: "#78716c", border: "1px solid rgba(0,0,0,0.08)" }}>{event.document.document_type}</span>}
                              </div>
                            )}
